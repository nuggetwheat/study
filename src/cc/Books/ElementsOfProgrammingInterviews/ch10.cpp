...
namespace study {
  ...
  pair<bool, int> checkBalanced(const std::unique_ptr<BSTNode<int>> &tree) {
    if (!tree)
      return {true, -1};
  ...
  bool isSymmetric(const std::unique_ptr<BSTNode<int>> &left,
                   const std::unique_ptr<BSTNode<int>> &right) {
    return (!left && !right) ||
      (left && right &&
  ...
  pair<int, BSTNode<int> *> lcaHelper(const std::unique_ptr<BSTNode<int>> &node,
  ...
  void sumRootToLeaf(const std::unique_ptr<BSTNode<int>> &node,
                     int *val, int *sum) {
    if (!node)
      return;
    *val = ((*val) << 1) | node->data;
  ...
  bool hasPathSum(const std::unique_ptr<BSTNode<int>> &node,
  ...
  BSTNode<int> *findKthNode(const std::unique_ptr<BSTNode<int>> &node,
                            size_t k, size_t *seen) {
    if (!node)
      return nullptr;
    BSTNode<int> *kthNode = findKthNode(node->left, k, seen);
    if (kthNode == nullptr) {
      if (++(*seen) == k)
        return node.get();
