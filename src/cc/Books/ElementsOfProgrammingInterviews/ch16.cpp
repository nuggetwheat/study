...
namespace study {
...
#if 0
  vector<unique_ptr<BSTNode<int>>> GenerateAllBinaryTrees(int num_nodes) {
    vector<unique_ptr<BSTNode<int>>> result;
    if (num_nodes == 0)
    ...
    for (int left_nodes=0; left_nodes<num_nodes; ++left_nodes) {
      ...
      for (auto & ltree : left_trees) {
        for (auto & rtree : right_trees) {
          unique_ptr<BSTNode<int>> tree(new BSTNode<int>(0));
  ...
  pair<int, int> computeDiameterHelper(const std::unique_ptr<BSTNode<int>> &node) {
